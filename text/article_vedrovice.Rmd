...
a burial (event) is simplified as a point location in a two dimensional space.
...
We presume that certain burial location is determined by other burial locations
and is not quite dependent on on other parameters.
In other words *first-order properties* are of little to no importance at all in
our point of view.
...
```{r spat-funs, fig.cap = "Functions *G*, *F*, *K* and *T*. *Dashed line* -- theoretical curve under CSR, *solid line* -- empirical curve, *gray area* -- envelope."}
knitr::include_graphics(here("plots", "pointprocess_fun.pdf"))
```

```{r neigh-sex, fig.cap = "Mean neighbors in a given sex category. *Gray area* -- density estimation for the simulated data, *vertical line* -- observed value."}
