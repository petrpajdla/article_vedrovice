...
Here we explore the relations between burials and bodies in terms of biological
...
proposed *levels* of neolithic identity, i.e. sex, age and localness.
...
these categories forming the *identity*, grave goods and spatial arrangement
of the cemetery.
...
Local Sr range is defined as two standard deviations from mean of Sr ranges of
juveniles.
...
<!-- Some other variables are removed because they are highly interdependent.-->

The variables fall into several categories.
Firstly there are biological and anthropological variables derived from
retrieved human skeletal remains including sex, age simplified into age groups,
various isotope ratios and body height.

Second large category of variables consists of data on artifacts present in the burials.
...
presence of polished and beveled stone axes or adzes, grinding slabs, pebbles,
bone tools and different kinds of personal ornaments.
The personal ornament includes beads made of spondylus shell or marble,
sponylus pendants or oval shaped buckles.
Presence of ochre pigment in the grave is also noted.
...
the sides and direction of the body and head.
...
Overall structure of the burial ground can be examined using either density based approaches
or methods based on distances between neighbors. <!--[@osullivan2010, 121-154]-->
...
On the other hand the methods based on distances between pairs and triples of neighbours
...
a graves location is either random or influenced predominantly by other (previous) grave locations and
...
based on the nearest neighbour distances and are commonly used
...
where categories are randomly assigned while preserving the overall
proportion of given categories and the neighborhood structure determined by the
graph.
The observed values of neighbors categories are compared against distributions
derived from 999 permutations of the simulated data.

<!-- Percolation analysis -->
Another approach considered for establishing neigborhood relations is by
constructing buffer zones around each of the burials.
The question arises how large the buffer zones should be.
To avoid this problem we utilise percolation analysis.
Percolation analysis is a method used to identify clusters of points based
...
nevertheless its effectivness and usefulness in studies with varied scales
is already proven [@maddison2020].
Percolation analysis is based on city clustering algorithm [@rozenfeld2008] and
its generalization utilizing Euclidean distance [@rozenfeld2011].
...
cluster sizes and other statistics.
This allows to identify distances at which clusters exhibit rapid grow (absorbing
...
Lastly, a modification of *K* and *G* functions for multitype point patterns
are used to explore the reationship between individual sex categories.
These take into account not all of the neighbors of the point pattern, but
count the statistic for pairs of categories in a given variable.
...
the boundaries of given envelopes at the distance of around 5 to 6 meters.
...
taking place.
The curve in Fig. \@ref(fig:perc-distance) reveals that most of the growth into
a single cluster happens at distances between approximately 4 and 6 meters.
The distances where relative stability of clusters emerges are labeled.
The clusters at these distances are shown in Fig. \@ref(fig:perc-plan).
