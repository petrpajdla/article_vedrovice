...
*Linearbandkeramik* (LBK) cemeteries in the Czech Republic.
...
Originally, this burial ground consisted of at least 96 LBK burials and
one Lengyel culture cremation [@ondrus2002a, 21].
Some of the LBK burials were disturbed by later settlement pits in the
area of the cemetery (4 burials), amateur excavations during the 19th century
(11 burial pits with recent fills) and later agricultural activities
(at least 6 burials disturbed by deep deep plowing) or combinations of these.
These burials are excluded from the analysis of artifact combinations due to missing and/or
incomplete data and only included in the spatial analysis.
This leaves us with a sample of of 76 well-preserved LBK burials in case of
the artifact combinations (79 % of the LBK burials) and
Most of the data was obtained from the original publication [@ondrus2002a],
...
non-adult (*n. a.*) and undetermined sex (*ind.*); see Figure \@ref(fig:plan-sex).
...
*Senilis*) and bodies of an undetermined age (*ind.*); see Figure \@ref(fig:plan-age).
...
where data on Sr values is missing, origin is not determined (*ind.*); see Figure \@ref(fig:plan-orig).
...
with burial locations as nodes (Figure \@ref(fig:plan-graphs)).
...
at the given radius there is strong evidence of a clustering process (Figure \@ref(fig:pp-funs)).
...
the most frequently used are packages from the *tidyverse* family [@wickham2019].
...
artifacts in different graves is not random (see Figure \@ref(fig:sstat)).
...
in graves are visualized in Figure \@ref(fig:cooc-nets) with
...
The results of functions based on nearest neighbors are reported in Figure \@ref(fig:pp-funs).
...
taking place; see Figure \@ref(fig:perc-distance).
...
The clusters at chosen distances are shown in Figure \@ref(fig:perc-plan).
...
most of the observed categories in Figure \@ref(fig:pp-crossk).
...
Gabriel graph (minimum number of neighbors scenario, Figure \@ref(fig:plan-graphs)),
...
we observe that in the largest cluster (no. 3, Figure \@ref(fig:perc-plan))
...
<!--
For relationships (see Figure ref(fig:nb-sex) and Tab. ref(tab:nb-tab-sex))
...
To enable re-use and reproducibility, a compendium containing data and code accompanying this paper is released at Zenodo data repository under DOI [10.5281/zenodo.4899652)](https://doi.org/10.5281/zenodo.4899652).
...
```{r label-pp-funs}
label_pp_funs <- paste("Functions G, F, K and T. Dashed line – theoretical curve under complete spatial randomness; solid line – empirical curve; gray area – envelope")
```
