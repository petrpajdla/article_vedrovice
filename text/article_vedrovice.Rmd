---
title           : "Spatial Patterns and grave goods differences at the Cemetery of Vedrovice (Czech Republic): A Resampling Approach to Identity Markers in the Early Neolihic."
# "A resampling approach to spatial arrangement and contents of burials in the Vedrovice cemetery"

author          : "Petr Pajdla"
    # name          : "Petr Pajdla"
    # affiliation   : "Department of Archaeology and Museology, Masaryk University, Brno, Czech Republic"
...
abstract        :
  ...
  attributes -- sex, age, geographical origin and their combinations.
...
Authors present gender in the Neolithic as being
of a very different nature than gender in the later prehistory.
It is not clearly expressed and
...
cemetery.-->
We focus predominantly on investigating non-random patterns in artifact
...
Our question is thus how grave goods relate to aforementioned
...
Further details on use and function of the artifacts were gathered from
@masclanslatorre2020.
...
Last but not least presence of ocher pigment in the grave is also noted.
...
Senilis) and bodies of undetermined age (*ind.*).
...
As mentioned previously we consider these three variables, sex, age and origin to be indicative of
...
To determine non-randomness of various artifact combinations in burials we conduct a
...
Original co-occurrence matrix is permuted 9999 times with constant
...
individual artifact with other artifacts
(for details see SI).
...
artifact combinations and similar approach is taken by @sosna2008 [346-347].
...
These methods are based on distances between pairs and triples of neighbors and
...
These functions are based on nearest neighbor distances and are commonly used
...
A modification of *K* function for multitype point pattern
...
Another approach we consider for establishing neighborhood relations is by
...
Points being within a given distance threshold (radius) are assigned into a cluster.
...
Observed numbers of occurrences deviating by more than two standard deviations from the mean of the
resampled results are regarded as strong evidence of non-random patterns being
present in given clusters.
To achieve stability of results and eliminate observations occurring due to
...
all the iterations are reported with expected numbers of occurrence and their
...
By counting co-occurrences of analyzed variables on simulated data matrices and comparing
the distributions of the statistics proposed by @manly1996 with the values calculated for
the original data set we can establish whether the overall pattern of artifacts
...
ceramic bowls and with 4 links adzes, non-local lithic tools, ocher pigment and
spondylus beads.
...
local lithic tools, globular pots, ceramic bowls and spondylus buckles with
...
More or less the same relations are present in case of combinations of sex and origin attributes.
...
Local male-sexed bodies of Adultus age category deviate in the artifact distributions
...
random process and do not show evidence for spatial clustering nor repellent functions
taking place.
On the contrary
...
This means that number of nearest neighbors at this and larger distances is
...
these is same as in case of multitype *K* for single categories.
...
Lastly at the distance radius 2.8 meters there are more local bodies in the central
...
of the artifact contents of the graves and position within the cemetery.
...
In case of grave goods analysis we explored the associations between artifacts and
...
individual artifact scale so to say but used an overall measure (Euclidean distance)
...
accentuated by local origin and age category, eminently structures the artifact assemblage
in the grave.
...
the cemetery and a finer scale is of much higher interest for our endeavor.
...
artifacts are well known across the LBK burial grounds,
...
First of all, we have shown that overall distribution of artifacts in the graves
...
This is caused by already mentioned artifact characteristic of male burials, the stone adze.
...
We have shown that not only sex that is traditionally perceived
...
```{r label-cooc-nets}
label_cooc_nets <- paste("Networks of variables frequently present together and exluding each other.",
...
knitr::include_graphics(here("text/figs", "cooc_nets_fin.png"))
```
