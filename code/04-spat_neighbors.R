...
ved_sf <- read_sf(here::here("data/temp", "layout.geojson")) %>% 
...
ved_exc <- read_sf(here::here("data/temp", "window.geojson")) %>% 
  st_set_crs(NA)
...
ved_gabriel_lines <- ved_gabriel %>% 
  graph2nb(row.names = ved_sf$id_burial) %>% 
...
g_gabriel <- ggplot() +
  ...
  geom_sf(data = select(ved_sf, -pres), shape = 21, fill = "gray60") +
  ...
  ggspatial::annotation_scale(plot_unit = "m",
                              location = "br", 
                              height = unit(0.2, "cm")) +
  labs(title = "(B) Gabriel graph", shape = "preservation", color = "preservation")
...
g_delaunay <- ggplot() +
  ...
  theme(legend.position = c(0.9, 0.8),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "(A) Delaunay triangulation", shape = "preservation")

g <- gridExtra::grid.arrange(g_delaunay, g_gabriel, nrow = 1)

ggsave(plot = g, filename = here::here("plots/plan_graphs.pdf"), 
...
pdf(here::here("plots/nb_sex.pdf"), width = 9, height = 12)
...
pdf(here::here("plots/nb_age.pdf"), width = 9, height = 12)
...
pdf(here::here("plots/nb_orig.pdf"), width = 6, height = 8)
