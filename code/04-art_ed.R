...
# ed_sa <- get_edistance(ved_input, "cat_sa", non_rand_vars, n_sim)
# ed_os <- get_edistance(ved_input, "cat_os", non_rand_vars, n_sim)
# ed_oa <- get_edistance(ved_input, "cat_oa", non_rand_vars, n_sim)
# ed_osa <- get_edistance(ved_input, "cat_osa", non_rand_vars, n_sim)


# output ------------------------------------------------------------------

# caution, this is awfully large (3 GB) and probably not neccessary...
# output <- list(sex = ed_sex,
#                age = ed_age,
#                orig = ed_orig,
#                sa = ed_sa,
#                os = ed_os,
...
#                osa = ed_osa)
# 
# write_rds(output, here("data/temp", "cooc_ed.RDS"))
...
groupwise_wilcoxon <- function(observed, expected) {
  obs <- observed %>%
    group_by(comb) %>%
    nest() %>%
    rename(obs = data)
...
# perform wilcoxon (Mann-Whitney-Wilcoxon test)
w_orig <- groupwise_wilcoxon(ed$orig$obs, ed$orig$exp)
w_sex <- groupwise_wilcoxon(ed$sex$obs, ed$sex$exp)
w_age <- groupwise_wilcoxon(ed$age$obs, ed$age$exp)
w_sa <- groupwise_wilcoxon(ed$sa$obs, ed$sa$exp)
w_os <- groupwise_wilcoxon(ed$os$obs, ed$os$exp)
w_oa <- groupwise_wilcoxon(ed$oa$obs, ed$oa$exp)
w_osa <- groupwise_wilcoxon(ed$osa$obs, ed$osa$exp)

w_res <- list(orig = w_orig,
              sex = w_sex,
              age = w_age,
              sa = w_sa, 
              os = w_os, 
...
write_rds(w_res, here("data/temp", "cooc_ed_wtest.RDS"))

# kruskal-wallis test ------------------------------------------------------

kwtest <- function(x) kruskal.test(value ~ parent, data = x)

groupwise_kruskal <- function(observed, expected) {
  bind_rows(obs = observed, exp = expected, .id = "parent") %>% 
    mutate(parent = factor(parent)) %>% 
    group_by(comb) %>% 
    nest() %>% 
    mutate(kwtest = map(data, kwtest),
           kwtest = map(kwtest, broom::tidy)) %>%
    select(-data) %>%
    ungroup() %>%
    unnest(kwtest) %>% 
    mutate(signif = if_else(p.value <= 0.05, "*", ""),
...
# perform kruskall-wallis test on the EDs
kw_orig <- groupwise_kruskal(ed$orig$obs, ed$orig$exp)
kw_sex <- groupwise_kruskal(ed$sex$obs, ed$sex$exp)
kw_age <- groupwise_kruskal(ed$age$obs, ed$age$exp)
kw_sa <- groupwise_kruskal(ed$sa$obs, ed$sa$exp)
kw_os <- groupwise_kruskal(ed$os$obs, ed$os$exp)
kw_oa <- groupwise_kruskal(ed$oa$obs, ed$oa$exp)
kw_osa <- groupwise_kruskal(ed$osa$obs, ed$osa$exp)

kw_res <- list(orig = kw_orig,
               sex = kw_sex,
               age = kw_age,
               sa = kw_sa, 
               os = kw_os, 
...
write_rds(kw_res, here("data/temp", "cooc_ed_kwtest.RDS"))
